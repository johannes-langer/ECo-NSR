...
def annotation_to_popper(
    ...
    """
    Convert ``matrix`` to a Popper-compatible format using the annotation information.
    ...
    ref_df = pd.read_csv(os.path.join(CWD, global_cfg.DATA.RavenMatrices.dataset_file))
    annos = get_xml(ref_df.iloc[matrix_idx, 1])  # pyright: ignore[reportArgumentType]

    if path is None:
        path = os.path.join(
            CWD, global_cfg.REASONING.POPPER.file_path, f"matrix_{matrix_idx}_annos"
        )
    ...
    with open(os.path.join(path, "bk.pl"), "w") as bk:
        ...
        answer_counter = 0
        for panel, _panel in zip(annos["Data"]["Panels"]["Panel"], matrix):
            # get a panel id
            panel_id = (
                _panel["type"] if "task" in _panel["type"] else f"answer{answer_counter}"
            )
            answer_counter += 1 if "answer" in panel_id else 0

            # write meta information
            bk.write(f"n_shapes({panel_id}, {len(_panel['locs'])}).\n")
            ...
            for comp in components:
                ...
                for idx, shape in enumerate(shapes):
                    ...
                    for dim, (k, val) in enumerate(shape.items()):
                        if k in ["@bbox", "@real_bbox", "@mask"]:
                            continue
                        else:
                            bk.write(
                                f"shape_prop({panel_id}_{idx}, latent{dim}, {val}).\n"
                            )
...
def representation_to_popper(
    ...
    if path is None:
        path = os.path.join(
            CWD, global_cfg.REASONING.POPPER.file_path, f"matrix_{matrix_idx}"
        )
